...
def the_aps_quickstep(pma, verify=True):
    ...
    #TODO: if we have a complete set of host-journal mappings then this check is pointless:
    if jrnl not in aps_journal_params:
        raise NoPDFLink(f'MISSING: Journal {pma.journal} not found in APS registry - attempted: none')


    # TODO: this is a very poor try-except block; if the point is to report that there's missing
    #       data, then we should test for the missing data and throw the error if so.  It's also
    #       misleading: there may be other reasons for NoPDFLink that are not to do with missing data.
